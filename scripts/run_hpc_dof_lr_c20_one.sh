...
RESULT="$BNQ_ROOT/dof_lr_e3_c20/$TRACE_STEM"
...
ACTUAL_VIS_DIR="$BNQ_ROOT/reference_e3_visibility"
...
visibility() {
  ...
  "$PYTHON" "$REPO/scripts/generate_standard_ply_visibility.py" \
    --trace "$trace_path" --visibility-source e3 \
    --require-model-assets-root "$MODEL_ROOT" --output "$output" \
...
"$PYTHON" "$REPO/scripts/generate_visibility_score_policy.py" \
  ...
  --variant dof_lr_e3_c20 --score contributing_fraction --threshold 0.20 \
