...
| 2026-08-03 | pending | 新增 CPU-only 双固定机位 6DoF 轨迹可视化：以对齐坐标系的中点完整 DanceNet3D GT PLY 为背景，仅对已有 prediction 的 2 个 CircleTurns trace 分别输出蓝色 GT-only 弧线和蓝色 GT + 黄色 LR predicted DoF 合成弧线；每个机位复用同一完整点云画布，移除底部说明并将合成图标题固定为 `Ground Truth DoF vs Predicted DoF`。 |
